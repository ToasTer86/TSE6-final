...
#define _VectorType long long
#define VECTOR_SIZE (20*20*20*20)
...
int main() {
	...
	ret = clSetKernelArg(kernel, 1, (VECTOR_SIZE/COMPUTE_UNITS)*sizeof(long), NULL);
	...
	{
		int nextWorkGroupSize = WORKGROUP_SIZE;
		for (int i = 0; i < iterations; i++)
		{
			nextWorkGroupSize *= WORKGROUP_SIZE;
		}
	...
	printf("Result from GPU: %lld\n", VectorPtrFromGpu[0]);
	printf("Result from CPU: %lld\n", cpuSum);
