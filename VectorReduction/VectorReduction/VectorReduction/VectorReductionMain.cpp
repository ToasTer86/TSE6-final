...
//#define VECTOR_SIZE (400*400)
//#define COMPUTE_UNITS 400
...
void CreateVector(_VectorType *startingPoint, long size)
{
...
int main() {
	...
	ret = clGetDeviceInfo(device_id, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(globalMemSize), &globalMemSize, NULL);
	checkError(ret, "Could not check global mem size");

	cl_ulong localMemSize;
	ret = clGetDeviceInfo(device_id, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(localMemSize), &localMemSize, NULL);
	checkError(ret, "Could not check local mem size");

	std::cout << "Global memory size: " << globalMemSize << std::endl;
	std::cout << "Local memory size: " << localMemSize << std::endl;
	std::cout << "Overall vector size: " << VECTOR_SIZE*sizeof(_VectorType) << std::endl;
	std::cout << "Max local memory alloc: " << WORKGROUP_SIZE*sizeof(cl_mem) << std::endl;
	...
	ret = clSetKernelArg(kernel, 1, WORKGROUP_SIZE*sizeof(int), NULL);
	...
	printf("Kernel execution %i took %f msec\n", iterations, totalKernelTime / 1000000);

	while (amtOfResults != 1)
	{
		int _globalSize = VECTOR_SIZE / pow(WORKGROUP_SIZE, iterations);
		...
		clGetEventProfilingInfo(timing_GPU, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &starttime, NULL);
		clGetEventProfilingInfo(timing_GPU, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &endtime, NULL);
		printf("Kernel execution %i took %f msec\n", iterations, (float)((endtime - starttime) / 1000000));
